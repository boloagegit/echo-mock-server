...
<tr :class="{'row-clickable':true}" @click="$emit('handle-rule-row-click', rule)" :title="t('rules.clickPreviewDblEdit')">
    ...
    <td class="col-cond col-hide-md" :title="condTooltip(rule)">
        <div v-if="condTags(rule).length" class="cond-list">
            <span class="cond-tag" :class="condTags(rule)[0].t" :title="condTags(rule)[0].v"><span class="cond-label">{{condTags(rule)[0].label}}</span>{{condTags(rule)[0].v}}</span>
...
<tr v-if="rulePreviewExpanded[rule.id]" :id="previewId" class="rule-preview-row">
    <td :colspan="previewColspan" class="rule-preview-cell">
        ...
        <div v-else-if="rulePreviewCache[rule.id]" class="rule-preview-content">
            <div class="pv-header">
                <button type="button" class="pv-id" @click="$emit('clip-copy', rulePreviewCache[rule.id].id)" :title="t('rules.copyId')"><i class="bi bi-fingerprint" aria-hidden="true"></i><span>{{rulePreviewCache[rule.id].id}}</span><i class="bi bi-clipboard pv-copy-icon" aria-hidden="true"></i></button>
...
<div class="pv-field" v-if="rulePreviewCache[rule.id].targetHost"><span class="pv-label">{{t('rules.targetPrefix')}}</span><code>{{rulePreviewCache[rule.id].targetHost}}</code></div>
                ...
                <div class="pv-body">
                    <div class="pv-body-header">
                        <span class="pv-label">{{rulePreviewCache[rule.id].faultType&&rulePreviewCache[rule.id].faultType!=='NONE'?t('rules.faultInjection'):t('rules.pvResponseContent')}} <ui-badge v-if="rulePreviewCache[rule.id]._isSse" class="badge badge-sse inline-badge">SSE</ui-badge></span>
