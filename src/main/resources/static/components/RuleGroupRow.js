...
<tr :class="{'row-clickable':true}" @click="$emit('handle-rule-row-click', rule)" :title="t('rules.clickPreviewDblEdit')">
    ...
    <td class="col-cond col-hide-md" :title="condTooltip(rule)">
        <div v-if="condTags(rule).length" class="cond-list">
            <span class="cond-tag" :class="condTags(rule)[0].t" :title="condTags(rule)[0].v"><span class="cond-label">{{condTags(rule)[0].label}}</span><span class="cond-pill-val">{{condTags(rule)[0].v}}</span></span>
            <span v-if="condTags(rule).length>1" class="cond-more" :title="condTooltip(rule)">+{{condTags(rule).length-1}}</span>
        </div>
    ...
    <td class="col-datetime col-hide-md">
        <div class="table-date-stack">
            ...
            <span class="rule-updated-by" :title="rule.updatedBy||t('rules.unknownOperator')"><i v-if="rule.updatedBy" class="bi bi-person" aria-hidden="true"></i>{{rule.updatedBy||t('rules.unknownOperator')}}</span>
        ...
        <div v-else-if="rulePreviewCache[rule.id]" class="rule-preview-content">
            <div class="pv-header">
                <div class="pv-header-context">
                    ...
                    <button type="button" class="pv-id" @click="$emit('clip-copy', rulePreviewCache[rule.id].id)" :title="rulePreviewCache[rule.id].id+' · '+t('rules.copyId')"><i class="bi bi-fingerprint" aria-hidden="true"></i><span>{{shortId(rulePreviewCache[rule.id].id)}}</span><i class="bi bi-clipboard pv-copy-icon" aria-hidden="true"></i></button>
                </div>
                <div class="pv-header-actions">
                    <ui-button size="compact" icon-only @click.stop="$emit('open-edit', rulePreviewCache[rule.id])" :disabled="!isLoggedIn" :title="t('rules.edit')" :aria-label="t('rules.edit')"><i class="bi bi-pencil" aria-hidden="true"></i></ui-button>
                    <ui-button size="compact" icon-only @click.stop="$emit('export-rule-json', rulePreviewCache[rule.id].id)" :title="t('rules.exportJson')" :aria-label="t('rules.exportJson')"><i class="bi bi-download" aria-hidden="true"></i></ui-button>
                    <ui-button v-if="rulePreviewCache[rule.id].responseId && isLoggedIn" size="compact" icon-only @click.stop="$emit('go-to-responses', rulePreviewCache[rule.id].responseId)" :title="t('rules.viewResponse')" :aria-label="t('rules.viewResponse')"><i class="bi bi-file-earmark-text" aria-hidden="true"></i></ui-button>
                    <ui-button variant="danger" size="compact" icon-only @click.stop="$emit('delete-rule', rulePreviewCache[rule.id].id)" :disabled="!isLoggedIn" :title="t('rules.delete')" :aria-label="t('rules.delete')"><i class="bi bi-trash" aria-hidden="true"></i></ui-button>
                </div>
            </div>
            <div class="pv-main">
                <div class="pv-fields ui-detail-panel">
                    <div class="pv-section-title">{{t('rules.pvSectionSettings')}}</div>
<div class="pv-field pv-field-wide" v-if="rulePreviewCache[rule.id].targetHost"><span class="pv-label">{{t('rules.targetPrefix')}}</span><code>{{rulePreviewCache[rule.id].targetHost}}</code></div>
                    ...
                    <div class="pv-field pv-field-wide" v-if="rulePreviewCache[rule.id].action==='FORWARD'"><span class="pv-label">{{t('rules.forward')}}</span><span class="pv-forward-target"><span>{{forwardTargetLabel(rulePreviewCache[rule.id])}}</span><strong v-if="rulePreviewCache[rule.id]._forwardTargetName">{{rulePreviewCache[rule.id]._forwardTargetName}}</strong><code v-if="forwardTargetEndpoint(rulePreviewCache[rule.id])">{{forwardTargetEndpoint(rulePreviewCache[rule.id])}}</code></span></div>
                    <div class="pv-field pv-field-wide" v-if="rulePreviewCache[rule.id].faultType&&rulePreviewCache[rule.id].faultType!=='NONE'"><span class="pv-label">{{t('rules.faultInjection')}}</span><span class="pv-result-value"><i class="bi bi-lightning" aria-hidden="true"></i>{{t('rules.fault_'+rulePreviewCache[rule.id].faultType)}}</span></div>
                    <div class="pv-field pv-field-wide" v-if="status?.scenariosEnabled&&rulePreviewCache[rule.id].scenarioName"><span class="pv-label">{{t('rules.pvScenario')}}</span><span class="pv-scenario-value" :title="t('rules.scenarioTooltip',{name:rulePreviewCache[rule.id].scenarioName,required:rulePreviewCache[rule.id].requiredScenarioState||'Started',newState:rulePreviewCache[rule.id].newScenarioState||rulePreviewCache[rule.id].requiredScenarioState||'Started'})"><strong>{{rulePreviewCache[rule.id].scenarioName}}</strong><code>{{rulePreviewCache[rule.id].requiredScenarioState||'Started'}}</code><i class="bi bi-arrow-right" aria-hidden="true"></i><code>{{rulePreviewCache[rule.id].newScenarioState||rulePreviewCache[rule.id].requiredScenarioState||'Started'}}</code></span></div>
                    ...
                    <div class="pv-field"><span class="pv-label">{{t('rules.pvProtected')}}</span><span class="pv-inline-value"><i class="bi pv-leading-icon" :class="rulePreviewCache[rule.id].isProtected ? 'bi-shield-fill-check text-success' : 'bi-shield'"></i>{{rulePreviewCache[rule.id].isProtected ? t('rules.pvYes') : t('rules.pvNo')}}</span></div>
                    <div class="pv-field pv-field-wide" v-if="!rulePreviewCache[rule.id].isProtected && daysLeft(rulePreviewCache[rule.id].createdAt, rulePreviewCache[rule.id].extendedAt, status?.cleanupRetentionDays) != null"><span class="pv-label">{{t('rules.pvDaysLeft')}}</span><span class="pv-inline-value"><ui-badge class="badge" :class="daysLeft(rulePreviewCache[rule.id].createdAt, rulePreviewCache[rule.id].extendedAt, status?.cleanupRetentionDays) <= 7 ? 'badge-warning' : 'badge-muted'">{{t('rules.daysLeft', {days: daysLeft(rulePreviewCache[rule.id].createdAt, rulePreviewCache[rule.id].extendedAt, status?.cleanupRetentionDays)})}}</ui-badge><ui-button v-if="isLoggedIn" variant="secondary" size="compact" class="pv-inline-action" @click.stop="$emit('extend-rule', rulePreviewCache[rule.id].id)"><i class="bi bi-calendar-plus"></i> {{t('rules.extend')}}</ui-button></span></div>
                    <div class="pv-field pv-field-wide" v-if="rulePreviewCache[rule.id].createdAt"><span class="pv-label">{{t('rules.pvCreated')}}</span><span>{{fmtTime(rulePreviewCache[rule.id].createdAt, false)}}</span></div>
                    <div class="pv-field pv-field-wide" v-if="rulePreviewCache[rule.id].updatedAt"><span class="pv-label">{{t('rules.pvUpdated')}}</span><span>{{fmtTime(rulePreviewCache[rule.id].updatedAt, false)}} · {{rulePreviewCache[rule.id].updatedBy||t('rules.unknownOperator')}}</span></div>
                    ...
                    <div class="pv-field pv-field-wide" v-if="rulePreviewCache[rule.id].responseHeaders"><span class="pv-label">{{t('rules.pvResponseHeaders')}}</span><code class="pv-code-copy" @click="$emit('clip-copy', rulePreviewCache[rule.id].responseHeaders)" :title="t('rules.clickToCopy')">{{rulePreviewCache[rule.id].responseHeaders}}</code></div>
                    <div class="pv-field pv-field-wide" v-if="Object.keys(parseTags(rulePreviewCache[rule.id].tags)||{}).length"><span class="pv-label">{{t('rules.pvTags')}}</span><span><span class="tag-badge" v-for="(v,k) in parseTags(rulePreviewCache[rule.id].tags)" :key="k">{{k}}:{{v}}</span></span></div>
                </div>
                <div class="pv-body ui-detail-panel">
                    <div class="pv-body-header">
                        <span class="pv-label ui-detail-panel-heading">{{rulePreviewCache[rule.id].faultType&&rulePreviewCache[rule.id].faultType!=='NONE'?t('rules.faultInjection'):t('rules.pvResponseContent')}} <ui-badge v-if="rulePreviewCache[rule.id]._isSse" class="badge badge-sse inline-badge">SSE</ui-badge></span>
                        <div v-if="!rulePreviewCache[rule.id].faultType||rulePreviewCache[rule.id].faultType==='NONE'" class="pv-body-tools">
